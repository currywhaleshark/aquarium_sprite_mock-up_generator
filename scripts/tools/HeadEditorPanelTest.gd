...
func _ready() -> void:
	...
	panel.set_numeric_parameter("head_bottom_flatness", -0.55)
	assert(abs(float(seen[0].get("head_bottom_flatness", 0.0)) + 0.55) < 0.001)
	panel.set_numeric_parameter("head_bottom_flatness", -2.0)
	assert(abs(float(seen[0].get("head_bottom_flatness", 0.0)) + 1.0) < 0.001)
	...
	assert(_has_numeric_slider(shark_panel, "snout_base"))
	assert(_has_numeric_slider(shark_panel, "snout_thickness"))
	assert(_has_numeric_slider(shark_panel, "snout_taper"))
	assert(_has_numeric_slider(shark_panel, "snout_curve"))
	...
	for shark_morph_key in ["shark_head_rear_height", "shark_head_rear_width", "shark_snout_tip_y", "shark_mouth_angle", "shark_mouth_arc"]:
		...
		assert(absf(shark_panel._default_numeric(shark_morph_key)) < 0.001)
	...
	var shark_head_rear_height_slider := _slider_for_key(shark_panel, "shark_head_rear_height")
	var shark_head_rear_width_slider := _slider_for_key(shark_panel, "shark_head_rear_width")
	var shark_snout_tip_slider := _slider_for_key(shark_panel, "shark_snout_tip_y")
	var shark_mouth_angle_slider := _slider_for_key(shark_panel, "shark_mouth_angle")
	var shark_mouth_arc_slider := _slider_for_key(shark_panel, "shark_mouth_arc")
	...
	assert(shark_head_rear_height_slider != null)
	assert(absf(shark_head_rear_height_slider.min_value + 0.4) < 0.001)
	assert(absf(shark_head_rear_height_slider.max_value - 0.8) < 0.001)
	assert(shark_head_rear_width_slider != null)
	assert(absf(shark_head_rear_width_slider.min_value + 0.4) < 0.001)
	assert(absf(shark_head_rear_width_slider.max_value - 1.0) < 0.001)
	assert(shark_snout_tip_slider != null)
	assert(absf(shark_snout_tip_slider.min_value + 0.35) < 0.001)
	assert(absf(shark_snout_tip_slider.max_value - 0.35) < 0.001)
	assert(shark_mouth_angle_slider != null)
	assert(absf(shark_mouth_angle_slider.min_value + 45.0) < 0.001)
	assert(absf(shark_mouth_angle_slider.max_value - 45.0) < 0.001)
	assert(absf(shark_mouth_angle_slider.step - 1.0) < 0.001)
	assert(shark_mouth_arc_slider != null)
	assert(absf(shark_mouth_arc_slider.min_value + 1.0) < 0.001)
	assert(absf(shark_mouth_arc_slider.max_value - 1.0) < 0.001)
	assert(absf(shark_mouth_arc_slider.step - 0.005) < 0.001)
	...
	assert(shark_head_body != null)
	assert(shark_snout_body != null)
	...
	shark_panel.set_numeric_parameter("head_top_flatness", 0.47)
	shark_panel.set_numeric_parameter("head_bottom_flatness", 0.36)
	shark_panel.set_numeric_parameter("head_left_flatness", 0.25)
	shark_panel.set_numeric_parameter("head_right_flatness", 0.14)
	shark_panel.set_numeric_parameter("head_bump_height", 0.31)
	shark_panel.set_numeric_parameter("head_bump_pos", 0.12)
	shark_panel.set_numeric_parameter("head_bump_width", 0.28)
	shark_panel.set_numeric_parameter("head_bump_angle", 64.0)
	...
	assert(abs(float(shark_seen[0].get("head_left_flatness", -1.0))) < 0.001)
	assert(abs(float(shark_seen[0].get("head_right_flatness", -1.0))) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_height", -1.0))) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_pos", 0.0)) + 0.2) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_width", 0.0)) - 0.18) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_angle", 0.0)) - 35.0) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_round", 0.0)) - 0.6) < 0.001)
	...
	assert(abs(float(shark_seen[0].get("shark_gill_slit_count", 0.0)) - 7.0) < 0.001)
	shark_panel.set_numeric_parameter("head_bump_height", 0.52)
	shark_panel.set_numeric_parameter("head_bump_pos", 0.18)
	shark_panel.set_numeric_parameter("head_bump_width", 0.32)
	shark_panel.set_numeric_parameter("head_bump_angle", 72.0)
	shark_panel.set_numeric_parameter("head_bump_round", 0.74)
	...
	assert(abs(float(shark_seen[0].get("head_top_flatness", 0.0)) - 0.18) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bottom_flatness", 0.0)) - 0.08) < 0.001)
	assert(abs(float(shark_seen[0].get("head_left_flatness", 0.0)) - 0.28) < 0.001)
	assert(abs(float(shark_seen[0].get("head_right_flatness", 0.0)) - 0.28) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_height", -1.0))) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_pos", 0.0)) + 0.2) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_width", 0.0)) - 0.18) < 0.001)
	assert(abs(float(shark_seen[0].get("head_bump_angle", 0.0)) - 35.0) < 0.001)
