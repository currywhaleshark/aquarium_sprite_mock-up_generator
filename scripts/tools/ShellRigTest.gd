...
func _ready() -> void:
	...
	var fish_shell_before := _mesh_vertices(fish_shell)
	fish.apply_pose(0.25)
	var fish_shell_after := _mesh_vertices(fish_shell)
	assert(_max_vertex_delta(fish_shell_before, fish_shell_after) > 0.001)
	...
	var shell_tail_center := _ring_center(fish_shell, fish._ring_index_by_id("tail_stem", 5), 28)
...
func _max_vertex_delta(before: PackedVector3Array, after: PackedVector3Array) -> float:
	var count := mini(before.size(), after.size())
	var max_delta := 0.0
	for i in count:
		max_delta = maxf(max_delta, before[i].distance_to(after[i]))
	return max_delta
