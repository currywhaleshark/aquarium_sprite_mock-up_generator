...
func _ready() -> void:
	...
	var pectoral_numeric_sliders: Dictionary = panel.get("numeric_sliders")
	assert(pectoral_numeric_sliders.has("pectoral_fin_spacing"))
	panel.set_numeric_parameter("pectoral_rigidity", 0.6)
	panel.set_numeric_parameter("pectoral_fin_spacing", 0.82)
	panel.select_slot("pelvic")
	var pelvic_numeric_sliders: Dictionary = panel.get("numeric_sliders")
	assert(pelvic_numeric_sliders.has("pelvic_fin_yaw"))
	panel.set_numeric_parameter("pelvic_fin_yaw", 34.0)
	panel.set("selected_slot", "dorsal_1")
	...
	assert(abs(float(seen_parameters[0].get("pectoral_fin_spacing", 0.0)) - 0.82) < 0.001)
	assert(abs(float(seen_parameters[0].get("pelvic_fin_yaw", 0.0)) - 34.0) < 0.001)
