...
func _ready() -> void:
	...
	panel.set_ring_parameter("bottom_flatness", -0.65)
	...
	assert(abs(float(rings[3].get("bottom_flatness", 0.0)) + 0.65) < 0.001)
	assert(abs(float(rings[3].get("sway_weight", 0.0)) - 0.5) < 0.001)
	panel.set_ring_parameter("bottom_flatness", -2.0)
	rings = _panel_rings(panel)
	assert(abs(float(rings[3].get("bottom_flatness", 0.0)) + 1.0) < 0.001)
